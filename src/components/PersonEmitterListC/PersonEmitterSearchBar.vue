...
export default {
  ...
  computed : {
    ...mapGetters('emitterCollector',[
      'getPersonEmitterPageNum',
      'getPersonEmitterLastPageNum',
  ...
  methods : {
    ...mapMutations('emitterCollector',['setPersonEmitterSearch']),
    // 엔터를 눌렀을 때 발생
    search(event){
      ...
      this.$router.push({ path: 'list', query: {
          page: 1 , keyword : this.getPersonEmitterSearch,
    ...
    keyUpEvent(event){
      ...
      this.setPersonEmitterSearch(content)
    }
  }
}
</script>
