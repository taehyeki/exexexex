...
export default {
  ...
  state: {
    ...
    calInfo: {
      selectedDay: new Date(Date.now() - new Date().getTimezoneOffset() * 60000)
        .toISOString()
        .substr(0, 10),
      notExistDays: [],
    },
  ...
  mutations: {
    ...
    setNowTarget(state, payload) {
      ...
      (state.mainTable.offsetSize = 0), (state.mainTable.nowTargetInfo = []);
    },
  ...
  actions: {
    ...
    async getMainInfo({ commit }) {
      ...
      const data = {
        params: JSON.stringify([{ USER_ID: 238 }]),
      };
